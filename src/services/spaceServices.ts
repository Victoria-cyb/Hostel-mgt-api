  ...
  createSpaceUsers = async (
    ...
    for (const input of inputs) {
        ...
        for (const sid of studentId) {
          ...
          if (!studentSpaceUser) {
            throw new CustomError(
              `Student with ID ${sid} not found or not a student in this space`,
            );
          }
        }
      }
      ...
      if (existingSpaceUser) {
        throw new CustomError(
          `User ${firstName} ${lastName} already a member of this space`,
        );
      }
    ...
    mappedSpace.users = (space.spaceUsers ?? []).map((su: any) => ({
      ...
      class: su.classId
        ? {
            id: su.classId,
            name: su.class?.name ?? "",
            createdAt: toISO(su.class?.createdAt),
          }
        : null,
      parent: su.parent
        ? {
            id: su.parent.id,
            firstName: su.parent.firstName,
            lastName: su.parent.lastName,
            email: su.parent.email ?? null,
          }
        : null,
    }));
